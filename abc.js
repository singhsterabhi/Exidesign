...
async function startSegmentation(win) {
  ...
  modal.innerHTML = `<div class="modal-header"></div><div class="modal-content"></div><div><button onclick="nextNode()" id='nextNode'>Next</button></div>`;
  ...
  $("#myModal").draggable();
  await findAllNodes(contentDocument);
  await gatherAllTextNodes(contentDocument);
...
async function popup(nodeVals) {
  return new Promise(function (resolve, reject) {
    let modalContent = "";
    for (let [key, value] of Object.entries(nodeVals)) {
...
async function findAllNodes(obj) {
  if (!obj) return;
  ...
  if (obj.tagName && !isHidden(obj)) {
    if (obj.tagName.toLowerCase() == "a") {
      ...
      linkNode = await getLinkNode(obj);
      if (linkNode != null) {
      ...
      markTextNodeVisisted(obj);
      inputNode = await getInputNode(obj);
      if (inputNode != null) {
...
async function gatherAllTextNodes(obj) {
  var n,
    walk = document.createTreeWalker(obj, NodeFilter.SHOW_TEXT, null, false);
  while ((n = walk.nextNode())) {
    if (
...
async function getLinkNode(obj) {
  var link = obj;
  var label = $(link).text();
  ...
  flag = false;
  await popup(linkNode)
  return linkNode;
}

async function getInputNode(obj) {
  var link = obj;
